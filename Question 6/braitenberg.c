...
int main() {
  ...
  while (wb_robot_step(time_step) != -1) {
    ...
    if (sensors_value[5] > SEUIL_OBSTACLE || sensors_value[2] > SEUIL_OBSTACLE ||
        ...
        // Make the robot turn right more gently to bypass the obstacle
        wb_motor_set_velocity(left_motor, max_speed * 0.5);  // Set left motor to move forward at half speed
        wb_motor_set_velocity(right_motor, -max_speed * 0.01);  // Set right motor to move backward at a reduced speed
    }
